 ...
 * 增加若设置了formatItem格式函数，则进行匹配的数据源从field改为formatItem后的结果进行匹配
 * 2016.10
...
;(function($){
	...
	var defaults = {
		...
		/**
		 * @desc 插件初始值指定，该值会与option.primaryKey字段进行匹配，若匹配到，则自动设置选中并高亮
		 * @type string 
		 */
		...
		field: 'name',
		/**
		 * @desc 查询字段，仅为使用URL(ajax)方式查询服务端时，设置后端查询的字段，不设置则默认使用field设置的字段
		 * @type string
		 */
	...
	SelectPage.prototype.setOption = function(option) {
		...
		option.searchField = (option.searchField === undefined) ? option.field: option.searchField;
		...
		//设置多字段排序
		//例:  [ ['id', 'ASC'], ['name', 'DESC'] ]
		option.orderBy = this.setOrderbyOption(option.orderBy, option.field);
			...
			if (typeof self.option.data == 'object') {
				...
				$.each(keyarr,function(index,row){
					for (var i = 0; i < self.option.data.length; i++) {
						if (self.option.data[i][self.option.primaryKey] == row) {
				...
				$.ajax({
					...
					data: {
						...
						pkey_name: self.option.primaryKey,
	...
	SelectPage.prototype.afterInit = function(self, data) {
		...
		if(self.option.multiple){//多选模式初始化
			...
			$.each(data,function(i,row){
				var item = {text:row[self.option.field],value:row[self.option.primaryKey]};
			...
			$(self.elem.hidden).val(row[self.option.primaryKey]);
			...
			success: function(returnData) {
				...
				for (var i = 0; i < json.cnt_page; i++) {
					for (var key in json.originalResult[i]) {
						if (key == self.option.primaryKey) {
							json.primaryKey.push(json.originalResult[i][key]);
						}
	...
	SelectPage.prototype.searchForJson = function(self, q_word, which_page_num) {
			...
			if(!self.option.multiple){
				...
				if(typeof(currentValue) != 'undefined' && $.trim(currentValue) != ''){
					...
					$.each(sorted,function(i,row){
						if(row[self.option.primaryKey] == currentValue){
							index = i + 1;
							return false;
						}
					});
		...
		for (i = start; i < end; i++) {
			...
			for (var key in sorted[i]) {
				if (key == self.option.primaryKey) {
					if (json.primaryKey === undefined) json.primaryKey = [];
					json.primaryKey.push(sorted[i][key]);
				}
				if (key == self.option.field) {
	...
	SelectPage.prototype.prepareResults = function(self, json, q_word, which_page_num) {
		...
		if (!json.primaryKey) json.primaryKey = false;

		//仅选择模式
		if (self.option.selectOnly && json.candidate.length === 1 && json.candidate[0] == q_word[0]) {
			$(self.elem.hidden).val(json.primaryKey[0]);
			this.setButtonAttrDefault();
		}
		//是否是输入关键词进行查找
		var is_query = false;
	...
	SelectPage.prototype.displayResults = function(self, json, is_query) {
		...
		var arr_primary_key = json.primaryKey;
